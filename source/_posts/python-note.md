...
- 全局命名空间保存全局变量，函数命名空间用来保存函数中的变量
  命名空间实际上就是一个字典，专门用来存储变量的字典，向字典中添加key-value相当于添加一个变量，但不建议这么做
- locals()
  - 获取当前作用域的命名空间
- globals()
  - 获取全局的命名空间
...
可以使用raise语句来抛出异常，raise后面跟个异常类或者异常的实例
```python
def add(a,b):
    if a < 0 or b <0:
        raise Exception('两个参数中不能有负数')
        # 告诉调用者调用时出现问题，希望你处理一下
    return a + b
```
可以自定义异常，只用创建一个继承Exception的类就行，类里啥都不用写。

### 文件（File）
- 通过Python程序对计算机中的各种文件进行增删改查
- 也叫I/O(Input/Output)
- 操作文件步骤
  1. 打开文件
  2. 读写文件
...
- 打开文件
  - open()
    ...
    - 文件种类：
      1. 纯文本文件
      2. 二进制文件
    - 默认以纯文本打开，默认的编码为None，处理文本文件时，必须要指定文件的编码
    - 需要指定你要做的操作，比如读、写、追加...如果不指定则默认只读
      - 'r'读
      - 'w'写，如果文件不存在，会创建文件，如果文件存在，会截断文件，截断就是删除原来文件中的所有内容
      - 'a'
      - 'r+'可读可写，文件不存在的话会报错
      - 'w+'写的基础上加了读
      - 'a+'追加且可读
    ...
    - 文件类型
      ...
      - 'b' 二进制文件
- 读取文件
  - read()方法，会把内容全部保存为一个字符串返回
    - 示例：`text_neirong = file_obj.read()`
    - 对于较大文件不要调用read文件，比如一口气给内存弄3个G文件进去
      - 可以接收size参数来指定读取字符数量，默认值为-1，表示全都读
        - 如果是二进制单位则是以字节为单位
      - 每次都是从上次读到的位置开始读
      - 如果不够，则会读完
      - 如果已经读完了，则会返回空串''
